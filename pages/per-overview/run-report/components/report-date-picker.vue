...
<template>
  ...
  >
    <view class="u-datetime-picker">
      ...
      <view class="u-flex u-picker-header2" v-if="mode == 'time'">
        <view
          class="u-flex u-picker-header2-start"
          :style="{
            'border-color': timePickerType == 1 ? '#009999' : '#CCCCCC',
          }"
          @tap="tapStartTimeAction"
          >{{ showStartTime }}</view
        >
        <view class="u-flex u-picker-header2-zhi">至</view>
        <view
          class="u-flex u-picker-header2-end"
          :style="{
            'border-color': timePickerType == 2 ? '#009999' : '#CCCCCC',
          }"
          @tap="tapEndTimeAction"
          >{{ showEndTime }}</view
        >
      </view>
...
<script>
import provinces from "@/uni_modules/uview-ui/libs/address/provinces.json";
import citys from "@/uni_modules/uview-ui/libs/address/citys.json";
import areas from "@/uni_modules/uview-ui/libs/address/areas.json";
/**
 * picker picker弹出选择器
...
export default {
  name: "u-picker",
  emits: ["update:modelValue", "input", "confirm", "cancel", "clearAction"],
  props: {
    // 通过双向绑定控制组件的弹出与收起
    value: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      timePickerType: 1, // 1：开始时间；2：结束时间
    ...
    getResult(event = null) {
      ...
      if (this.mode == "time") {
        if (this.timePickerType == 1) {
          ...
          if (this.showEndTime == "结束时间") {
            this.timePickerType = 2;
          } else {
            if (event) this.$emit(event, this.showStartTime, this.showEndTime);
            this.close();
          }
        } else if (this.timePickerType == 2) {
          this.showEndTime =
            result.year + "/" + result.month + "/" + result.day;
          if (this.showStartTime == "开始时间") {
            this.timePickerType = 1;
          } else {
            if (event) this.$emit(event, this.showStartTime, this.showEndTime);
            this.close();
