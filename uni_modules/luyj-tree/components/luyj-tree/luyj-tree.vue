...
export default {
  ...
  emits: ["send-value", "click-item", "back-tree"],
  data() {
    return {
      // 导航条
  ...
  methods: {
     ...
     */
    toItemChildren(item, realHasChildren, fromParent) {
      if (!fromParent) {
        this.$emit("click-item", item, realHasChildren); // 点击导航栏事件
      }
      // 不包含子级,不执行任何操作
      if (!realHasChildren) {
        return;
     ...
     */
    backTree(item, index) {
      this.$emit("back-tree", item, index);
      let that = this;
      if (index == -1) {
        // 全部
     ...
     */
    backConfirm() {
      this.$emit("send-value", this.newCheckList, "back");
    },
    // ======================================== 公共方法 ===============================================================
    /**加载Tree值
